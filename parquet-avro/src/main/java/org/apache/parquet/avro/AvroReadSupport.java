...
public class AvroReadSupport<T> extends ReadSupport<T> {
  ...
  public ReadContext init(
    ...
    if (requestedProjectionString != null) {
      Schema avroRequestedProjection =
          new Schema.Parser().setValidateDefaults(false).parse(requestedProjectionString);
  ...
  public RecordMaterializer<T> prepareForRead(
    ...
    if (metadata.get(AVRO_READ_SCHEMA_METADATA_KEY) != null) {
      ...
      avroSchema =
          new Schema.Parser().setValidateDefaults(false).parse(metadata.get(AVRO_READ_SCHEMA_METADATA_KEY));
      ...
      avroSchema = new Schema.Parser()
          ...
          .parse(keyValueMetaData.get(AVRO_SCHEMA_METADATA_KEY));
    } else if (keyValueMetaData.get(OLD_AVRO_SCHEMA_METADATA_KEY) != null) {
      // use the Avro schema from the file metadata if present
      avroSchema = new Schema.Parser()
          .setValidateDefaults(false)
          .parse(keyValueMetaData.get(OLD_AVRO_SCHEMA_METADATA_KEY));
    } else {
